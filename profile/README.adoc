...
The runtime environment for the applications and services is called uPlatform. Platforms contain the contains core services (uDiscovery, uSubscription, uTwin) and event dispatchers (uStreamer) required to implement the protocol as seen in <<uprotocol-platform>> below.
